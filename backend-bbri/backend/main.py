...
async def upload_dataset(file: UploadFile = File(...)):
    ...
    return {
        ...
        "rows": len(df_model),
        "last_date": str(df_model["Date"].iloc[-1]),
    }
...
async def run_prediction():
    ...
    # =====================================================
    # METADATA DATASET KESELURUHAN (SEBELUM SPLIT)
    # =====================================================
    total_rows_original = len(df)
    start_date_original = df["Date"].min().strftime("%Y-%m-%d")
    end_date_original = df["Date"].max().strftime("%Y-%m-%d")
    ...
    (metrics_all, svr_pred_all, rf_pred_all, svr_model_all, rf_model_all) = (
    ...
    (
        ...
        svr_model_selected,
        rf_model_selected,
    ) = train_and_evaluate_models(X_train_selected, y_train, X_test_selected, y_test)
    ...
    latest_X_scaled = pd.DataFrame(scaler.transform(latest_X), columns=FEATURES)
    ...
    all_test_close = model_df["Close"].iloc[split_idx:].values
    ...
    actual_prices = all_test_close * np.exp(y_test.values)

    svr_sel_prices = all_test_close * np.exp(svr_pred_selected)
    rf_sel_prices = all_test_close * np.exp(rf_pred_selected)

    svr_all_prices = all_test_close * np.exp(svr_pred_all)
    rf_all_prices = all_test_close * np.exp(rf_pred_all)
    ...
    return {
        ...
        "dataset_info": {
            ...
            "date_range": f"{start_date_original} - {end_date_original}",
        },
        "prediction": {
        ...
        "metrics": {"All_Features": metrics_all, "Selected_Features": metrics_selected},
        "chart_data": {
